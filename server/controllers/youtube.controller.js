...
const fetchAndFormatVideos = async (searchParams) => {
    const searchResponse = await makeYoutubeRequest(YOUTUBE_SEARCH_URL, searchParams);
...
exports.getPopularVideos = async (req, res) => {
  ...
  try {
    ...
    if (user && user.feedPreferences && user.feedPreferences.length > 0) {
      ...
      videoData = await fetchAndFormatVideos({ part: 'snippet', q: searchQuery, type: 'video', maxResults: 24, pageToken });
    } else {
      logger.info('Fetching default popular feed.');
      const response = await makeYoutubeRequest(YOUTUBE_VIDEOS_URL, {
        part: 'snippet,contentDetails,statistics', chart: 'mostPopular', regionCode: 'US', maxResults: 24, pageToken
      });
      videoData = { items: response.data.items, nextPageToken: response.data.nextPageToken };
    }
    ...
    return res.status(status).json({ success: false, message: 'Failed to fetch videos from YouTube.' });
  }
};
...
exports.searchVideos = async (req, res) => {
  ...
  try {
    const videoData = await fetchAndFormatVideos({ part: 'snippet', q, type: 'video', maxResults: 24, pageToken });
    res.status(200).json({ success: true, data: videoData });
  } catch (error) {
    const status = error.response ? error.response.status : 500;
    const message = error.response?.data?.error?.message || 'Server Error';
    logger.error(`searchVideos failed: ${message}`);
    res.status(status).json({ success: false, message: 'Failed to search YouTube videos.' });
  }
};