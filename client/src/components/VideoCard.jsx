...
const VideoCard = ({ video, isYouTubeVideo = false, index, isSavedPage = false, onUnsave }) => {    if (!video || !video.id) {
        return null;
    }
    ...
    const handleSaveVideo = (e) => {
        ...
        api.saveVideo(videoToSave)
            .then(() => toast.success('Video saved!'))
            .catch(err => toast.error(err.response?.data?.message || 'Could not save video.'));
    };
    ...
    return (
        <Link to={videoData.url} className={styles.cardLink} style={{ animationDelay: `${index * 50}ms` }}>
            <div className={styles.thumbnailWrapper}>
                <img src={videoData.thumbnailUrl} alt={videoData.title} className={styles.thumbnail} />
                {isSavedPage ? (
                    <button onClick={handleUnsaveVideo} className={`${styles.actionButton} ${styles.unsaveButton}`}>
                        <FiTrash2 />
                    </button>
                ) : isYouTubeVideo ? (
